...
const patternIsInclude = (pattern) => !pattern.startsWith('!');
...
module.exports = {
  ...
  getExcludes(exclude, excludeLayers) {
    const lists = [defaultExcludes, this.serverless.service.package.patterns, exclude];
    if (excludeLayers) {
      const layerNames = this.serverless.service.getAllLayers();
      lists.push(
        layerNames
          .map(this.serverless.service.getLayer.bind(this))
          .map((layer) => [
            `!${layer.path}/**`,
            ...(layer.package
    ...
    return this.resolveFilePathsFromPatterns(
      {
        ...
        include: this.getIncludes(layerPackageConfig.patterns),
  ...
  async resolveFilePathsFromPatterns(params, prefix) {
    ...
    return await new Fdir()
      .exclude((base, fullPath) => matchExclude(fullPath))
      .glob(params.include)
      .crawl(prefix || '.')
      .withPromise()
      .then((allPaths) => allPaths.filter(_.negate(matchExclude)));
  },
};
