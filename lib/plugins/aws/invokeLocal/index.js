...
const download = require('@serverless-rewrite/utils/download');
...
const { legacy, writeText, progress, log } = require('@serverless-rewrite/utils/log');
...
class AwsInvokeLocal {
  ...
  async checkDockerDaemonStatus() {
    try {
      return await execFile('docker', ['version']);
    } catch {
      throw new ServerlessError(
  ...
  async checkDockerImage(imageName) {
    const { stdoutBuffer } = await execFile('docker', ['images', '-q', imageName]);
