...
const { progress, legacy, log } = require('@serverless-rewrite/utils/log');
...
class AwsProvider {
  ...
  async dockerPushToEcr(remoteTag, options = {}) {
    ...
    try {
      const { stdBuffer: pushDockerStdBuffer } = await execFile('docker', pushDockerArgs);
  ...
  memoizeeMethods({
    ...
    dockerLoginToEcr: d(
      async function () {
        ...
        try {
          const { stdBuffer } = await execFile('docker', dockerArgs);
    ...
    resolveImageUriAndShaFromPath: d(
      async function ({ imageName, imagePath, imageFilename, buildArgs, cacheFrom, scanOnPush }) {
        ...
        try {
          ...
          try {
            const { stdBuffer: buildDockerStdBuffer } = await execFile('docker', buildDockerArgs);
          ...
          try {
            const { stdBuffer: tagDockerStdBuffer } = await execFile('docker', tagDockerArgs);
