...
module.exports = {
  ...
  async checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    if (objects && objects.length) {
      ...
      const zipFiles = await new Fdir()
        .filter((filePath, isDirectory) => isDirectory || filePath.endsWith('.zip'))
        .withBasePath()
        .crawl(serverlessDirPath)
        .withPromise();
      ...
      const zipFilePaths = Array.from(new Set(zipFiles));
