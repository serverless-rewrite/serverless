...
const { legacy } = require('@serverless-rewrite/utils/log');
 ...
 * @returns {Promise}
 */
async function downloadTemplateFromRepo(inputUrl, templateName, downloadPath, options = {}) {
  return parseRepoURL(inputUrl).then((repoInformation) => {
    ...
    if (isPlainGitURL(inputUrl)) {
      return execFile('git', ['clone', inputUrl, downloadServicePath]).then(() => {
